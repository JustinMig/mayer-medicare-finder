...
type MedicarePlanRow = {
  ...
  cms_pbp_extracted: BenefitDetails | null
  cms_pbp_source_url: string | null
  cms_pbp_imported_at: string | null
...
type CountyJoinRow = { county_name: string; medicare_plans: MedicarePlanRow | MedicarePlanRow[] | null }

function isDsnp(plan: MedicarePlanRow) { return /d-snp|dual/i.test(`${plan.snp_type || ''} ${plan.plan_name}`) }
function normalizedPlan(joined: CountyJoinRow['medicare_plans']) { return Array.isArray(joined) ? joined[0] || null : joined || null }
function detailString(details: BenefitDetails | null, key: string) { const value = details?.[key]; return typeof value === 'string' && value.trim() ? value.trim() : null }
function summaryLines(details: BenefitDetails | null) { const lines = details?.q1_summary_lines; return Array.isArray(lines) ? lines.filter((line): line is string => typeof line === 'string') : [] }
function summaryLine(details: BenefitDetails | null, pattern: RegExp) { return summaryLines(details).find((line) => pattern.test(line))?.replace(/^\s*[•*-]\s*/, '').trim() || null }
...
function moneyString(value: string | number) {
  ...
  return new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: numeric % 1 === 0 ? 0 : 2, maximumFractionDigits: 2 }).format(numeric)
}
...
function annualAllowanceFromText(value: string | null) {
  ...
  for (const pattern of patterns) for (const match of value.matchAll(pattern)) { const amount = Number(match[1].replace(/,/g, '')); if (Number.isFinite(amount) && amount >= 0) candidates.push(amount) }
  return candidates.length ? `${moneyString(Math.max(...candidates))} / year` : null
}
...
function recurringAllowanceFromText(value: string | null) {
  ...
  return `${moneyString(match[1])} / ${frequency.startsWith('month') ? 'month' : frequency.startsWith('quarter') ? 'quarter' : 'year'}`
}
...
function recurringAllowance(details: BenefitDetails | null, prefix: 'otc' | 'food', fallback: string | null) {
  ...
  if (amount) return `${moneyString(amount)}${frequency ? ` / ${frequency.replace(/^\s*per\s+/i, '').trim().toLowerCase()}` : ''}`
...
function benefitSegment(value: string | null, label: string) {
  ...
  return segment ? segment.slice(segment.indexOf(':') + 1).trim() || null : null
}
function firstBenefitSegment(value: string | null) { if (!value) return null; const segment = value.split(/\s*·\s*/g).map((part) => part.trim()).find(Boolean); if (!segment) return null; const separator = segment.indexOf(':'); return separator >= 0 ? segment.slice(separator + 1).trim() || null : segment }
function compactVisionSummary(exam: string | null, eyewear: string | null, allowance: string | null) { const parts: string[] = []; if (exam) parts.push(`Eye exam: ${exam}`); if (allowance) parts.push(`Eyewear: ${allowance}`); else if (eyewear) parts.push(`Eyewear: ${eyewear}`); return parts.length ? parts.join(' · ') : null }
function compactHearingSummary(exam: string | null, aids: string | null, allowance: string | null) { const parts: string[] = []; if (exam) parts.push(`Exam: ${exam}`); if (allowance) parts.push(`Hearing allowance: ${allowance}`); else if (aids) parts.push(`Hearing aids: ${aids}`); return parts.length ? parts.join(' · ') : null }
function partBGiveback(details: BenefitDetails | null) { const amount = detailString(details, 'part_b_credit_monthly') || detailString(details, 'part_b_giveback_monthly'); return amount ? `${moneyString(amount)} / month` : null }
...
export async function GET(request: NextRequest) {
  ...
  const { data, error } = await supabase.from('medicare_plan_counties').select(`
    county_name,
    medicare_plans!inner(
      ...
      benefit_details, cms_pbp_extracted, cms_pbp_source_url, cms_pbp_imported_at,
      cms_source_date, q1_source_url, source_note,
      structured_benefits_source, structured_benefits_verified_at
    )
  `).eq('state', 'MS').ilike('county_name', county).order('county_name')
  ...
  if (medicaid === 'none') plans = plans.filter((plan) => !isDsnp(plan))
  else plans = plans.filter((plan) => !isDsnp(plan) || plan.medicaid_level_status !== 'verified' || (plan.medicaid_levels || []).some((level) => level.toLowerCase() === medicaid))

  plans.sort((a, b) => {
    if (medicaid !== 'none') { const dualDifference = Number(isDsnp(b)) - Number(isDsnp(a)); if (dualDifference) return dualDifference }
    return a.carrier.localeCompare(b.carrier) || a.plan_name.localeCompare(b.plan_name) || a.plan_id.localeCompare(b.plan_id)
  })

  const results = plans.map((plan) => {
    const details = { ...(plan.benefit_details || {}), ...(plan.cms_pbp_extracted || {}) }
    ...
    const hearingAnnualAllowance = detailString(details, 'hearing_annual_allowance') || annualAllowanceFromText(plan.hearing_benefit)
    ...
    return {
      ...
      hearing_annual_allowance: hearingAnnualAllowance,
      ...
      hearing_summary: compactHearingSummary(hearingExam, hearingAids, hearingAnnualAllowance),
      otc_allowance: otcAllowance,
      food_allowance: foodAllowance,
      inpatient_hospital: plan.inpatient_hospital || detailString(details, 'inpatient_hospital'),
      ambulance_copay: plan.ambulance_copay || detailString(details, 'ambulance_copay') || summaryLine(details, /ambulance/i),
      emergency_room_copay: plan.emergency_room_copay || detailString(details, 'emergency_room_copay') || summaryLine(details, /emergency room|emergency care/i),
      urgent_care_copay: plan.urgent_care_copay || detailString(details, 'urgent_care_copay') || summaryLine(details, /urgent care/i),
      ...
      benefit_source: plan.cms_pbp_imported_at ? 'CMS PBP Benefits 2026 JSON' : plan.structured_benefits_source || 'CMS/Q1 reference',
      medicaid_match_status: !isDsnp(plan) ? 'not_required' : plan.medicaid_level_status === 'verified' ? 'verified' : medicaid === 'none' ? 'not_selected' : 'needs_verification'
    }
  })

  return NextResponse.json({ county: exactRows[0]?.county_name || county, medicaid, plan_year: 2026, results, count: results.length, cms_source_date: results.find((plan) => plan.cms_source_date)?.cms_source_date || '2026-08-10', part_b_standard_premium_2026: PART_B_STANDARD_PREMIUM_2026, part_d_oop_cap_2026: PART_D_OOP_CAP_2026 }, { headers: { 'Cache-Control': 'private, max-age=120, stale-while-revalidate=600' } })
}
