...
export default function MedicarePlanFinderLean() {
  ...
  const comparisonRows = useMemo(() => [
    ...
    { label: 'Net standard Part B after giveback', get: (p: MedicarePlan) => shortDisplay(p.part_b_net_standard_cost) },
    ...
    { label: 'Ambulance', get: (p: MedicarePlan) => shortDisplay(p.ambulance_copay) },
    { label: 'Emergency room', get: (p: MedicarePlan) => shortDisplay(p.emergency_room_copay) },
    { label: 'Urgent care', get: (p: MedicarePlan) => shortDisplay(p.urgent_care_copay) },
  ...
  return <div className={s.workspace}>
    <section className={s.searchPanel}>
      <div className={s.sectionTitle}><div><span>STEP 1</span><h2>Client location & eligibility</h2><p>County controls plan availability. ZIP and radius are used only for doctor searches.</p></div></div>
      <form className={s.locationGrid} onSubmit={searchPlans}>
        <label>Mississippi county<input list="ms-counties-lean" value={county} onChange={(e) => setCounty(e.target.value)} placeholder="Example: Alcorn" autoComplete="off" /><datalist id="ms-counties-lean">{MISSISSIPPI_COUNTIES.map((name) => <option value={name} key={name} />)}</datalist></label>
        ...
        <label>Doctor radius<select value={radius} onChange={(e) => resetLocationDependent(e.target.value, 'radius')}><option value="5">5 miles</option><option value="10">10 miles</option><option value="25">25 miles</option><option value="50">50 miles</option><option value="100">100 miles</option></select></label>
    ...
    {payload && <section className={s.resultsSection}>
      <div className={s.resultsHeader}><div><span>STEP 3</span><h2>{payload.count} plans in {payload.county} County</h2><p>CMS plan data updated {sourceDate(payload.cms_source_date)} · select up to 4 plans to compare.</p></div><div className={s.filters}><label>Carrier<select value={carrier} onChange={(e) => setCarrier(e.target.value as (typeof CARRIERS)[number])}>{CARRIERS.map((item) => <option key={item}>{item}</option>)}</select></label>{selectedDoctors.length > 0 && <button type="button" className={`${s.filterButton} ${onlyAllDoctors ? s.filterOn : ''}`} onClick={() => setOnlyAllDoctors((v) => !v)}>{onlyAllDoctors ? '✓ ONLY IN-NETWORK DOCTORS' : 'ONLY IN-NETWORK DOCTORS'}</button>}</div></div>
      ...
      <div className={s.planGrid}>{filteredPlans.map((plan) => { const selected = selectedPlanIds.includes(plan.id); const networkPlan = network?.plans?.[plan.id]; return <article key={plan.id} className={`${s.planCard} ${selected ? s.planSelected : ''}`}><div className={s.planCardHeader}><div><span className={s.carrier}>{plan.carrier}</span><h3>{plan.plan_name}</h3><small>{plan.plan_key} · {plan.plan_type || 'Medicare Advantage'}{plan.is_dsnp ? ' · D-SNP' : ''}</small></div><label className={s.compareCheck}><input type="checkbox" checked={selected} onChange={() => togglePlan(plan.id)} disabled={!selected && selectedPlanIds.length >= 4} /><span>{selected ? 'Selected' : 'Compare'}</span></label></div>
        ...
        <div className={s.cardFooter}>{plan.q1_source_url && <a href={plan.q1_source_url} target="_blank" rel="noreferrer">Plan reference ↗</a>}<span>{plan.benefit_source || 'Verified plan data'}</span></div>
      </article> })}</div>{!filteredPlans.length && <div className={s.emptyBox}>No plans match the current filters.</div>}
    </section>}
